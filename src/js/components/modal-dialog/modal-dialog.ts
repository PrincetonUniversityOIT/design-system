...
const INPUT_SELECTORS_EXCL_CLOSE = 'a[href]:not([disabled]), button:not([disabled]):not(.pjz-modal-button__close), textarea:not([disabled]), input[type="text"]:not([disabled]), input[type="radio"]:not([disabled]), input[type="checkbox"]:not([disabled]), select:not([disabled])';
...
export class ModalDialogBehavior extends Behavior {
    ...
    init(root: ParentNode) {
        this.select(MODAL_WRAPPER_SELECTOR, root).forEach((wrapper) => {
            wrapper.classList.remove('pjz-modal__wrapper--visible')
        });
    }
    ...
    onAction(event: Event) {
        ...
        this.select(MODAL_WRAPPER_SELECTOR, modalEl).forEach((wrapper) => {
            ...
            if (button.matches(MODAL_BUTTON_SELECTOR) || event instanceof KeyboardEvent) {
                wrapper.classList.toggle('pjz-modal__wrapper--visible');
                if (wrapper.classList.contains('pjz-modal__wrapper--visible')) {
